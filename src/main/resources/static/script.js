// API Configuration
const API_BASE_URL = '';  // Empty since we're already in /api context
...
document.addEventListener('DOMContentLoaded', function() {
    // Check for regular login first
    if (authToken && currentUser) {
        showApp();
        loadRooms();
    } 
    // Check for guest session
    else if (sessionStorage.getItem('guestToken') && sessionStorage.getItem('guestUser')) {
        authToken = sessionStorage.getItem('guestToken');
...
function showGuestModal() {
    console.log('showGuestModal called');
    const modal = document.getElementById('guest-modal');
    if (modal) {
        modal.style.display = 'flex';
        console.log('Modal display set to flex');
        
        // Focus management - focus the first input or the modal itself
        setTimeout(() => {
...
function closeGuestModal() {
    console.log('closeGuestModal called');
    const modal = document.getElementById('guest-modal');
    if (modal) {
        modal.style.display = 'none';
        console.log('Modal display set to none');
    } else {
        console.error('Guest modal not found for closing');
    }
}
...
async function handleGuestLogin(event) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/guest/enter`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            currentUser = {
                id: data.guestId,
                username: data.guestId,
                displayName: data.displayName,
                isGuest: true
            };
            
            // Store guest session (will be lost on page reload)
            sessionStorage.setItem('guestToken', authToken);
            sessionStorage.setItem('guestUser', JSON.stringify(currentUser));
            
            closeGuestModal();
            showToast('Welcome, ' + data.displayName + '!', 'success');
            showApp();
            loadPublicRooms(); // Load only public rooms for guests
        } else {
            const error = await response.json();
            console.error('Guest login error response:', error);
            showToast(error.message || 'Failed to start guest session', 'error');
        }
    } catch (error) {
...
function showApp() {
    ...
    // Update display name and show guest indicator
    const displayName = currentUser.displayName + (currentUser.isGuest ? ' (Guest)' : '');
    document.getElementById('user-display-name').textContent = displayName;
    
    // Hide create room button for guests
    if (currentUser.isGuest) {
        const createBtn = document.querySelector('button[onclick="showCreateRoomModal()"]');
        if (createBtn) {
...
// Load only public rooms for guest users
async function loadPublicRooms() {
    try {
        ...
        if (response.ok) {
            const rooms = await response.json();
            displayRooms(rooms, true); // Pass true to indicate guest mode
        } else {
            showToast('Failed to load rooms', 'error');
        }
    } catch (error) {
...
function displayRooms(rooms, isGuestMode = false) {
    const roomList = document.getElementById('room-list');
    roomList.innerHTML = '';
    
    if (isGuestMode && rooms.length === 0) {
        roomList.innerHTML = '<p class="no-rooms">No public rooms available. Ask a registered user to create a room!</p>';
        return;
    }
    ...
    rooms.forEach(room => {
        ...
        roomElement.innerHTML = `
            ...
            ${isGuestMode ? '<span class="room-badge">Public Room</span>' : ''}
        `;
